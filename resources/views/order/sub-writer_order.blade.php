							...
							<table  class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
								...
								<tbody class="allData">
									...
									<tr id="order_{{ $order->id }}" class="{{ ($order->is_read == 1) ? 'bold-row' : '' }}" onclick="markAsRead('{{ $order->id }}')">										
										...
										<td>
                                            @if($order->writer_status == 'Other')
											<span class="badge badge-light-primary fs-7 fw-bold " style="background:#44f2e4; color:black">{{$order->writer_status}}</span>
											...
											<span class="badge badge-light-warning fs-7 fw-bold" style="background:#eaea00; color:black">{{$order->writer_status}}</span>
                                            @elseif($order->writer_status == 'Delivered')
											<span class="badge badge-light-success fs-7 fw-bold" style="background:green; color:white">{{$order->writer_status}}</span>
                                            @elseif($order->writer_status == 'Cancelled')
